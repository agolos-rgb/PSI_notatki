...
#Import danych----
setwd ("A:\\narzedzia informatyczne\\PSi\\zajecia_02") # domy?lnym katalogiem b?dzie C:/Data/project 
kraje_1 = read.table("kraje_makro_1.csv", header=TRUE, sep=",", dec=".")
kraje_2 = read.table("kraje_makro_2.csv", header=TRUE, sep=",", dec=".")

#install.packages("readxl") 	# uruchom linijk?, a potem j? zakomentuj
...
#   ?	podgl?d danych
# ?	porz?dkowanie nazw kolumn (je?li potrzeba)
# ?	porz?dkowanie typ?w danych (je?li potrzeba np. z tekstu na liczby)
# ?	porz?dkowanie brak?w danych (usuwanie lub uzupe?nianie brakuj?cych warto?ci)
# ?	czyszczenie danych (je?li potrzeba)
# 
# 
# [poni?sze sekcje mo?esz skopiowa? i wklei? do nowego skryptu R, ?eby wykonywa? kod]
...
# Funkcja merge() ??czy dwie ramki danych/tabele po wsp?lnej kolumnie (kluczu) - dzia?a analogicznie jak 
# WYSZUKAJ.PIONOWO w Excelu

# Przyk?adowo: merge(ramka1, ramka2, by.x="kolumna1", by.y="kolumna2")


#Laczenie ramek danych w jedna----
